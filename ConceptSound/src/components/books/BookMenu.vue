...
const addTag = () => addAlert((tagName) => store.addTag(tagName));
</script>
  ...
  <ion-menu side="start" menuId="BookMenu" contentId="BookMenuContent">
    ...
    <ion-content class="ion-padding">
      ...
      <inline-elements class="ion-margin-top">
        <ion-input label="Tytuł:" mode="md" fill="outline" label-placement="stacked" maxlength="50"
                   v-model="store.book.title"
                   @focusout="store.setBookTitle($event.target.value)"></ion-input>
        <ion-button class="ion-margin-start" shape="round" @click="addTag()" fill="clear">
